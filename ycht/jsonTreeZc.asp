...
strSaleAreaTree = "[{name: '请选择类别',expanded:true,children: ["
if rs.RecordCount <> 0 then
	Do while not rs.eof or rs.bof
		strSaleAreaTree = strSaleAreaTree & "{name:'"&rs("departmentName")&"',soleId:'"&rs("id")&"'"
		'查询二级类别
		...
		if rs1.RecordCount <> 0 then
			...
			Do while not rs1.eof or rs1.bof
				strSaleAreaTree = strSaleAreaTree &"{name:'"&rs1("departmentName")&"',leaf:true,soleId:'"&rs1("id")&"'"
				...
				if rs2.RecordCount <> 0 then
					...
					Do while not rs2.eof or rs2.bof
						strSaleAreaTree = strSaleAreaTree &"{name:'"&rs2("departmentName")&"',leaf:true,soleId:'"&rs2("id")&"'},"
						rs2.movenext
					loop
	...
	strSaleAreaTree = strSaleAreaTree & "{name:'无',soleId:'',leaf:true}"
end if
strSaleAreaTree = strSaleAreaTree &"]}]"
conn.close
