...
Ext.onReady(function(){
	...
	maps.centerAndZoom(new BMap.Point(116.84947,38.29603), 4);
					...
					Ext.Ajax.request({
						...
						success:function(req){
							...
							for(var i=0;i<json.length;i++)
							{
								...
								//html+=json[i].info+"<br>";
								html+="客户电话："+json[i].iconname+"<br>";
								html+='业务员:'+json[i].url+'<br>';
...
//	{header:'备注',dataIndex:'info'}
	...
	{header:'客户电话',dataIndex:'iconname',width: 500}
	//,//项目名称
...
function addAllCustomerMarker(ids,businessnames,addresss,goodstypes,lnglats,icons,iconnames,iconadresss,urls,infos){
	...
	grid.on('rowmousedown',function(a,b,c){
		if(customerJsonStore.getAt(b))
		{
			...
			html+="客户电话："+customerJsonStore.getAt(b).get("iconname")+"<br>";
			html+='业务员：'+customerJsonStore.getAt(b).get("url")+'<br>';
